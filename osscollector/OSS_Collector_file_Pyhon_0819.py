...
gitCloneURLS= currentPath + "/sample_python_0819" 			# Please change to the correct file (the "sample" file contains only 10 git-clone urls)
...
def main():
	with open(gitCloneURLS, 'r', encoding = "UTF-8") as fp:
		...
		for eachUrl in lines:
			...
			# try:
			if os.path.isdir(clonePath + repoName):
				print(clonePath + repoName)
				os.system("rm -rf "+clonePath + repoName)
